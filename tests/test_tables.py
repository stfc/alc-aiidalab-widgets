...
def test_generic_arraydata_table():
    ...
    assert "32.00000" in widget.children[2].value
    assert "0.000000" in widget.children[2].value
    assert widget.children[0].value == "Array_1"

    widget.array_selector.index = 1
    assert "32" in widget.children[2].value
    assert widget.children[0].value == "Array_2"
    assert "1" in widget.children[2].value
    assert "Array 2" in widget.children[2].value

    widget.array_selector.index = 2
    assert widget.children[0].value == "Array_3"
    assert "32" in widget.children[2].value
    assert "1" in widget.children[2].value
    assert "Array_3" not in widget.children[2].value
