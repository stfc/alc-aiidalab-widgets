...
class AiiDADatabaseQueryWidget(ipw.VBox, HasTraits):
    """Widget for AiiDA database searching."""

    data_object = Instance(Node, allow_none=True)

    def __init__(self, title: str = "", query: list | None = None):
        """AiiDADatabaseQueryWidget constructor.
        ...
        title: str
            The widget's title
        query: List | None
            A list of AiiDA node types to search for in the database, if None will
            default to search for CalcJobNodes and WorkChainNodes.
        """
        if query is None:
            query = []
        self.title = title
        ...
        self.drop_down = ipw.Dropdown(
            options={
                "All": self.query_type,
                **{node.__name__.strip("Node"): node for node in self.query_type},
            },
            # value="All",
        ...
        self.results.observe(self._on_select_node, names="value")
    ...
    def _on_select_node(self, _) -> None:
